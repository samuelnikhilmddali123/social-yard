...
// 13. Remove {step >= 3 && ( that appears around Total Budget
i = lines.findIndex(l => l.includes('{step >= 3 && ('));
if (i !== -1) {
    lines.splice(i, 1);
    let end = lines.findIndex((l, index) => index > i && l.includes(')}'));
    if (end !== -1) {
        lines.splice(end, 1);
    }
}

// 14. Close Panel 4 and remove AnimatePresence closures
