...
const seedMedianCorridor = async () => {
  try {
    ...
    // Exactly on the MG Road CENTER DIVIDER
    const startLat = 16.50532;
    const startLng = 80.6312;
    
    // Precise linear steps to follow the center median line
    const latStep = -0.000078; 
    const lngStep = 0.00044;
    ...
    console.log(`Successfully deployed ${createdScreens.length} screens EXACTLY on the MG Road Divider.`);
    ...
    console.log('✅ Median corridor deployment complete.');
    process.exit(0);
  } catch (err) {
