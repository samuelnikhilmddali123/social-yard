...
const seedCorrectedCorridor = async () => {
  try {
    ...
    const locationName = "Vijayawada PWD Grounds (Main Road)";
    
    // Aligned along MG Road beside Ambedkar Statue & PWD Grounds
    const startLat = 16.5067;
    const startLng = 80.6312;
    
    // Incremental step to follow the road stretch
    const latStep = -0.00008; 
    const lngStep = 0.0004;
    ...
    for (let i = 1; i <= 10; i++) {
      ...
      // Side A (FRONT - Faces traffic coming from Benz Circle towards Statue/PCR)
      const screenA = {
        name: `Pole ${i} - Front`,
      ...
      // Side B (BACK - Faces traffic coming from Statue/PCR towards Benz Circle)
      const screenB = {
        name: `Pole ${i} - Back`,
        location: locationName,
    ...
    console.log(`Created ${createdScreens.length} screens along the MG Road Corridor (beside PWD Grounds).`);
