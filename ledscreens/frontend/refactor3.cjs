...
replaceBlock('{/* Stepper progress indicator */}', '</div>', ''); // First div close
replaceBlock('{/* Main Split Layout */}', '{/* Main Split Layout */}', ''); // Just in case, won't do anything

const trackerStartIdx = lines.findIndex(l => l.includes('{/* Stepper progress indicator */}'));
const mainSplitIdx = lines.findIndex((l, i) => i > trackerStartIdx && l.includes('{/* Main Split Layout */}'));
if (trackerStartIdx !== -1 && mainSplitIdx !== -1) {
  lines.splice(trackerStartIdx, mainSplitIdx - trackerStartIdx);
}

// Remove AnimatePresence wrapper around steps
...
if (leftColIdx !== -1) {
  lines[leftColIdx] = lines[leftColIdx].replace('lg:h-[calc(100vh-160px)] lg:min-h-[680px] flex flex-col justify-between', 'space-y-8 flex flex-col');
}

// Replace Step 1 completely
replaceBlock(
...
// Step 3 (Schedule) - Remove wrappers and buttons
const step3Start = lines.findIndex(l => l.includes('{/* STEP 3: SCHEDULE & TIME SELECTION */}'));
if (step3Start !== -1) {
  // Remove {step === 3 && ( and <motion.div...
  lines.splice(step3Start + 1, 2, '              <div className="bg-white border border-slate-200 rounded-[24px] p-6 shadow-sm space-y-5">');
  ...
  // Remove back/continue buttons for step 3
  replaceBlock('<div className="pt-4 border-t border-slate-100 flex items-center justify-between">', '</motion.div>', '</div>');
  
  // The first replaceBlock replaces the FIRST occurrence. But since we removed the ones before it, the first occurrence is now in step 3!
}

// Step 4 (Upload) - Remove wrappers and buttons
const step4Start = lines.findIndex(l => l.includes('{/* STEP 4: UPLOAD CREATIVE */}'));
if (step4Start !== -1) {
  lines.splice(step4Start + 1, 2, '              <div className="bg-white border border-slate-200 rounded-[24px] p-6 shadow-sm space-y-5">');
  replaceBlock('<div className="pt-4 border-t border-slate-100 flex items-center justify-between">', '</motion.div>', '</div>');
}

// Step 5 (Review & Pay) - Remove wrappers and buttons
const step5Start = lines.findIndex(l => l.includes('{/* STEP 5: REVIEW & PAY */}'));
if (step5Start !== -1) {
  lines.splice(step5Start + 1, 2, '              <div className="bg-white border border-slate-200 rounded-[24px] p-6 shadow-sm space-y-5">');
  // There might not be a continue button at the end of step 5, but there is a 'Pay & Launch Campaign' button. We want to keep it, but remove the </motion.div>
  const backBtnIdx = lines.findIndex((l, i) => i > step5Start && l.includes('<ChevronLeft size={14} /> Back'));
  if (backBtnIdx !== -1) {
    // Remove the back button wrapper
    lines.splice(backBtnIdx - 2, 4); // removing the button element completely
  }
  
  // Find </motion.div> for step 5 and remove it
  const motionDivEnd = lines.findIndex((l, i) => i > step5Start && l.includes('</motion.div>'));
  if (motionDivEnd !== -1) {
    lines.splice(motionDivEnd, 1);
  }
}
...
// Fix trailing )} which are left over from step === X && (
// The pattern is: )}
for (let i = 0; i < lines.length; i++) {
  if (lines[i].trim() === ')}' && lines[i - 1] && (lines[i - 1].trim() === '</div>' || lines[i - 1].trim() === '</div>' || lines[i - 1].trim() === '</div>')) {
    lines[i] = ''; // clear it out
  }
}

// One last pass to remove ANY lingering )} that come right after a </div> in the left column.
let openCount = 0;
for (let i = 0; i < lines.length; i++) {
