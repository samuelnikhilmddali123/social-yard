...
if "%ERRORLEVEL%"=="0" (
    ...
    start "Cloudflare Tunnel" /min "c:\led_server\cloudflared.exe" tunnel run --protocol http2 --token eyJhIjoiMjQ2ZDQxN2Q1YzNkMGRlYTA3NDA4ZDFlYzAyNmMzOGMiLCJ0IjoiNjczNTJiOWEtNjFhZC00ODJlLWE0ZDAtZWZiYTMyZThjZDNmIiwicyI6Ik5UaGpNR0kzTmprdE1EVTNOaTAwTVRWakxUbG1NMkl0T1dReFkyWTFZVE0xWlRCaCJ9
)

:: 2. Start CCTV Edge Agent Relay in background if not running
echo [INFO] Starting CCTV Edge Relay Agent in background...
start "CCTV Edge Relay" /min cmd /c "c:\led_server\start_edge_agent.bat"
